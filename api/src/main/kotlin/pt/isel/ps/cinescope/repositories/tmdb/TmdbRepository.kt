...
class TmdbRepository {
    ...
    fun fetchQuery(input: String, page: Int) =
        fetch("search/multi?$TMDB_API_KEY&page=$page&query=$input")
            ...
            .onErrorMap { e -> throw e.cause ?: InternalServerErrorException("external api error: ${e.localizedMessage}") }
            .block()
    ...
    fun getSeasonWatchProviders(sid: Int, season: Int) =
        fetch("tv/$sid/season/$season/watch/providers?$TMDB_API_KEY")
            .bodyToMono<WatchProviders>().onErrorMap { e -> throw e.cause ?: InternalServerErrorException("external api error: ${e.localizedMessage}") }
            .block()
