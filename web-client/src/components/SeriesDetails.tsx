...
import { BACKDROP_IMAGE_DOMAIN, IMAGE_DOMAIN, addToList, getCookie, handleError, showChecked, showCheckedEpisode, showUserLists } from "../utils/Tools";
import {
    ...
    async function addState(ev) {
        ...
        if (state == "Add State") {
            ...
            showLists.results.map(list =>
                showChecked(seriesStateInfo, list.id)
            )
                                ...
                                showLists.results.map(list =>
                                    <div className="showListInfo">
                                        <div className="listInfo">
                                            ...
                                            <input id={`${list.id}`} name={list.name} type="checkbox" checked={showChecked(seriesStateInfo, list.id)}
                                                onClick={(event) => addToList(event, list.id, +serieId, addSerieToList, getSeriesUserData, deleteSerieFromList)}
                                                className="checkmarkLists" />
                                        </div>
                                    </div>
                                )
                            }
                    ...
                    <div className="posterDiv">
                        ...
                        {userToken && <div className="buttonsDiv">
                            ...
                            <button className="buttonList"
                                onClick={() => showUserLists(getSeriesLists, getSeriesUserData, setSeriesStateInfo, setShowLists, setShowListsDiv, +serieId)}>
                                Add to List
                            </button>
                        </div>}
                    </div>
                                    ...
                                    <select name="season" id="season" onChange={showSelectedSeason} className="dropdown">
                                        {
                                            serie.serieDetails.seasons.map(season =>
                                                <option id={`${season.season_number}`} value={season.season_number}
                                                    className="selected" selected={false}>Season {season.season_number}</option>
                                            )
                                        }
                                    </select>
                                </div>
                                    ...
                                    <div className="seasonEpisodes">
                                        {
                                            showInfoSeason.seasonDetails.episodes.map(episode =>
                                                <div className="episodeInfo"
                                                    onClick={() => navigate(`/series/${serieId}/season/${showInfoSeason.seasonDetails.season_number}/episode/${episode.episode_number}`)}>
                                                    ...
                                                    <div className="episodeButtons" >
                                                        {userToken && <div className="addEpisode">
                                                            ...
                                                            <input id={`${showInfoSeason.seasonDetails.season_number}${episode.id}`} name={episode.name} type="checkbox"
                                                                checked={showCheckedEpisode(watchedList, episode.episode_number, showInfoSeason.seasonDetails.season_number)}
                                                                onClick={(event) => addToWatchedList(event, episode.id, episode.episode_number, showInfoSeason.seasonDetails.season_number)}
                                                                className="checkmarkEpisodes" />
                                                        </div>}
                                                    </div>
                                                </div>
                                            )
